...
"""
    @endpoint struct ... end
    @endpoint [func(...)] -> [struct ... end] -> [input] -> url -> resulttype
...
"arrow forms" joined by `->`. These arrow forms represent the flow of
information:
1. The creation of a request, from a function call (optional)
2. The `struct` used to represent the request data (optional, autogenerated)
3. Any input provided with the request (if applicable)
...
definition with a matching signature will be generated.
...
Such paths usually consist of a path component, and optionally a query component.
...
function generate_funcalls(einfo; mod::Module)
    ...
    args, kwargs = if Meta.isexpr(first(einfo.func.args), :parameters)
        einfo.func.args[3:end], einfo.func.args[1].args
    else
    ...
    f1 = :($fcall = perform(Request{$reqmethod}(globalconfig(Var($mod)), $estrucform)))
...
function parse_endpoint_url(url::String; kwargs...)
    ...
    isnothing(query) && return interp_curlies(path; percentescape=true, kwargs...), nothing
